...
$dbh = DBI->connect("dbi:SQLite:dbname=$dbname","","", {'PrintError'=>0}) || die "Cannot connect: $DBI::errstr";
$dbh->{AutoCommit}=0;	# Enable transactions
my $db_parser = DateTime::Format::DBI->new($dbh);
my $dt = DateTime->now();
my $Date = $db_parser->format_datetime($dt);
...
foreach my $name ( sort keys %planets ) {
    ...
    for my $glyph (@$glyphs) {
        ...
        $insert_statement = "INSERT INTO glyph_history (mydate, glyph, quantity, planet) values (\"$Date\", \"$glyph->{type}\", \"$glyph->{quantity}\", \"$name\")";
